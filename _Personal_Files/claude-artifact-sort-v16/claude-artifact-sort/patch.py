...
with open('content (2).js', 'r', encoding='utf-8') as f:
    text = f.read()

# 1. Constants (insert after SCAN_INTERVAL)
CONSTANTS = """
...
"""
text = text.replace('const SCAN_INTERVAL = 1200; // ms between rescans', 'const SCAN_INTERVAL = 1200; // ms between rescans\n' + CONSTANTS)

# 2. Storage helpers (insert after storageSet)
STORAGE = """
  function fmtNow() {
    const now = new Date();
...
"""
text = re.sub(r'(function storageSet\(key, val\) \{[\s\S]*?return new Promise\(r => chrome\.storage\.local\.set\(\{ \[k\]: val \}, r\)\);\n  \})', r'\1\n' + STORAGE, text)
...
text = re.sub(r'          <div style="display:flex;gap:5px;align-items:center;margin-top:5px">\s*<button id="cas-summarise"[\s\S]*?<button id="cas-inject"[\s\S]*?</div>', PANEL_BTNS.strip('\n'), text)
...
text = re.sub(r'          <div style="display:flex;gap:5px;align-items:center;margin-top:10px">\s*<button id="cas-chat-summarise"[\s\S]*?<button id="cas-chat-inject"[\s\S]*?</div>', CHAT_BTNS.strip('\n'), text)


# 4. Summarise and Inject replacements
SUMMARISE_FN = """
  /**
...
"""
text = re.sub(r'  async function performSummarise\(statusTarget\) \{.*?\}(?=\n\n  function buildFlyoutPrompt)', SUMMARISE_FN.strip('\n'), text, flags=re.DOTALL)
  ...
  async function performInjection(statusTarget) {
    ...
    try {
      ...
      for (const [idx, artifact] of artifacts.entries()) {
        if (!artifact.isSidebar && PLATFORM !== 'gemini') continue;
        let rawValue;
      ...
      if (typeof refreshSummariseBadge === 'function') refreshSummariseBadge();
    } catch (e) {
...
"""
text = re.sub(r'  async function performInjection\(statusTarget\) \{.*?\}(?=\n\n  async function renderProjectView)', INJECT_FN.strip('\n'), text, flags=re.DOTALL)


# 5. Tag Picker and Modal
TAG_PICKER = """
  ...
  async function buildTagPickerModal(key, type) {
    ...
    for (const [cat, tags] of Object.entries(currentSubtags)) selectedSubtags[cat] = new Set(Array.isArray(tags) ? tags : []);
...
"""
text = text.replace('  function renderList(items, listElem, statusElem, dataNoteElem, dataSummaryElem) {', TAG_PICKER.strip('\n') + '\n\n  function renderList(items, listElem, statusElem, dataNoteElem, dataSummaryElem) {')

# 6. Event listeners replacements and insertions (cas-tag-artifacts, cas-tag-chat)
# Replace existing cas-summarise and cas-inject with our mode-aware setup and inject the tag ones
EVENTS_REPLACE = """    // ── Tag Artifacts button (for current chat, no isSidebar restriction for Gemini) ──
    document.getElementById('cas-tag-artifacts')?.addEventListener('click', async () => {
      const items = scanForFileList().filter(i => i.source === 'generated' && i.data.name && (PLATFORM === 'gemini' || i.isSidebar));
      if (items.length === 0) { alert('No artifacts found. Open the artifact panel first.'); return; }
    ...
    // ── ↓ Summarise / Tag (Mode-aware) ──────────────────────────────────
    const btnSum = document.getElementById('cas-summarise');
    if (btnSum) {
      const newBtn = btnSum.cloneNode(true);
      if (btnSum.parentNode) btnSum.parentNode.replaceChild(newBtn, btnSum);
    ...
    // ── ↓ Inject — reads paste field first, then last DOM response ──
    document.getElementById('cas-inject')?.addEventListener('click', async () => {
      performInjection(status);
    });"""

text = re.sub(r'    // ── ↓ Summarise — fills.*?\}\);', EVENTS_REPLACE, text, flags=re.DOTALL)

# 7. Gemini Canvas Sorter extension (inserted before final `})();`)
GCS = """
  ...
  if (PLATFORM === 'gemini') {
    ...
    function getGeminiChatTitle() {
      return document.querySelector('[class*="conversation-title"]')?.textContent?.trim() || document.querySelector('h1')?.textContent?.trim() || document.title.replace(' - Gemini','').trim() || getChatId();
    }
    function injectGeminiToggleButtons() {
      ...
      group.innerHTML = `<button id="gcs-panel-toggle-main" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:6px;color:#aaa;border:1px solid rgba(255,255,255,0.15);background:rgba(40,44,52,0.6);cursor:pointer;" title="Open Canvas Sorter">⬡</button><button id="gcs-panel-toggle-summary" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:6px;color:#aaa;border:1px solid rgba(255,255,255,0.15);background:rgba(40,44,52,0.6);cursor:pointer;" title="GCS Chat Summary">⌬</button>`;
      group.querySelector('#gcs-panel-toggle-main').addEventListener('click', () => {
        buildPanel(); setTimeout(() => document.getElementById('cas-scan')?.click(), 100);
      });
      group.querySelector('#gcs-panel-toggle-summary').addEventListener('click', () => {
        if (!document.getElementById('cas-flyout-panel')) if (typeof buildFlyout === 'function') buildFlyout();
        ...
        if (el) {
          ...
          if (!isVisible && typeof renderChatSummaries === 'function') renderChatSummaries();
        }
      });
    ...
    function scanGeminiForFileList() {
      ...
      if (typeof recordFirstSeen === 'function') recordFirstSeen(items); 
      recordVersionUpdate(items); registerGeminiChat(items);
      return items;
    }
    ...
    function onGeminiChatChange() {
      ...
      if (typeof renderChatSummaries === 'function') renderChatSummaries();
      if (typeof renderCurrentChatSummary === 'function') renderCurrentChatSummary();
    }
    ...
    setInterval(() => {
      ...
      if (sidebar && !sidebar.querySelector('#cas-sidebar-bar')) {
        const items = scanGeminiForFileList();
        if (items.length > 0 && typeof injectSidebarSortBar === 'function') injectSidebarSortBar(items);
      }
    }, 1200);

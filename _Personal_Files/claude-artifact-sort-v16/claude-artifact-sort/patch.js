...
let text = fs.readFileSync('content (2).js', 'utf8');
...
text = text.replace(/<div style="display:flex;gap:5px;align-items:center;margin-top:5px">\s*<button id="cas-summarise".*?<\/button>\s*<button id="cas-inject".*?<\/button>\s*<\/div>/, PANEL_BTNS);
...
text = text.replace(/<div style="display:flex;gap:5px;align-items:center;margin-top:10px">\s*<button id="cas-chat-summarise".*?<\/button>\s*<button id="cas-chat-inject".*?<\/button>\s*<\/div>/, CHAT_BTNS);
...
`;
text = text.replace(/async function performSummarise\(statusTarget\) \{[\s\S]*?\}(?=\s*function buildFlyoutPrompt)/, SUMMARISE_FN.trim());
...
`;
text = text.replace(/async function performInjection\(statusTarget\) \{[\s\S]*?\}(?=\s*async function renderProjectView)/, INJECT_FN.trim());
...
`;
text = text.replace('  function renderList(items, listElem, statusElem, dataNoteElem, dataSummaryElem) {', TAG_PICKER.trim() + '\n\n  function renderList(items, listElem, statusElem, dataNoteElem, dataSummaryElem) {');

// 6. Events
const EVENTS_REPLACE = `    // ── Tag Artifacts button (for current chat, no isSidebar restriction for Gemini) ──
    ...
    if (btnSum) {
      ...
      newBtn.addEventListener('click', () => {
        ...
        performSummarise(document.getElementById('cas-status'), mode);
      });
    }

    // ── ↓ Inject — reads paste field first, then last DOM response ──
    document.getElementById('cas-inject')?.addEventListener('click', async () => {
      performInjection(document.getElementById('cas-status'));
    });`;

text = text.replace(/    \/\/ ── ↓ Summarise — fills[\s\S]*?\}\);[\s\n]*\/\/ ── ↓ Inject — reads[\s\S]*?\}\);/, EVENTS_REPLACE);
...
`;
text = text.replace('})();', GCS.trim() + '\n})();');

fs.writeFileSync('content.js', text, 'utf8');
console.log('Node patch successfully completed.');

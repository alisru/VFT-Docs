...
**The +i recursion:**
After c, +i restarts the full Q→q→c cycle — the output of c becomes the new subject, Q fires again at the next depth level.
...
**Scoring — the contrastive differential:**
Every interrogative is a differential — not "what is the value at this plane" but "what is the value relative to the available field of alternatives at this plane." A plane passes at the strength of its fill relative to the best available fill in that field.
...
Each Q plane contains 7 sub-interrogatives: the full {WHO, WHAT, WHERE, WHY, HOW, CAUSE, EFFECT} of that plane's claim. Track each as:
...
**Variance fields — the possibility cloud:**
Baseline n_υ and n_ψ are derived only from locked (P/F) vectors. Blanks generate swing ranges:
...
**z-profile — ambiguity localisation:**
The total z collapses the distribution. The z-profile preserves it:
...
**z at sub-judgement depth:**
Each node at q and c level carries its own local z. Ambiguity can be localised to a specific address — not just "Q1 is blank-heavy" but "Q1q3 (WHERE of WHO) is where operator context is missing." Report local z at each node when drilling.

**Conditional convergence:**
A high-z concept does not simply fail or pass — it converges conditionally. State conditions explicitly:

> "Converges IF [blank vector address] = [specific context fill]"

**Vector Coherence Magnitude (V_Qn):**
Blanks excluded. Passes weighted by fractional magnitude.

> V_Qn = sum(P_magnitudes) / (sum(P_magnitudes) + n_F)
> If P + F = 0: V_Qn = 1.0 (perfect coherence as pure potential)
...
**Object State blanking — named procedure:**
When the subject is inanimate, a raw concept, or a tool without an assigned operator:
...
**The objective ideal:**
The objective ideal is the highest-coherence fill available at each plane relative to the full field of alternatives. For each Q plane: given everything available to this actor at this time, what was the best possible selection? The objective ideal coordinate is built from those optimal fills. This gives ΔH a structural definition, not a qualitative comparison.

**Hypocrisy Gap:**
> ΔH = ‖ Vector_Ideal − Vector_Action ‖

Vector_Ideal = coordinate built from optimal plane fills, not stated intent.
Vector_Stated = stated intent coordinate (separate comparison if needed, not the ΔH standard).
...
Evidence standards: [A] Stated ideal · [B] Actions within context · [C] Objective ideal · [+D...n if applicable]
...
Convergences: n/7 · Divergences: n/7 · Blanks: n/49
z-profile: [B_Q1, B_Q2, B_Q3, B_Q4, B_Q5, B_Q6, B_Q7]
Phantom fills detected: [none / list]

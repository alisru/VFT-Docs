...
def scrape_via_tavily(url):
    api_key = os.environ.get("TAVILY_API_KEY", "")
    if not api_key:
        return None, None, "Tavily API key not found"
    try:
        payload = {
            "api_key": api_key,
            "urls": [url],
        ...
        res = requests.post("https://api.tavily.com/extract", json=payload, timeout=10)
        ...
        return None, None, f"Tavily error: status {res.status_code}"
...
def cluster_candidate_stories(candidates, similarity_threshold=0.35):
    """
    Groups candidates into topic clusters based on entity and title token overlap.
    De-duplicates identical syndicates, selects up to 3 distinct publisher perspectives per cluster,
    and attaches multi-source comparison dossiers.
    """
    ...
    STOPWORDS = set([
        ...
        "than", "too", "very", "s", "t", "can", "will", "don", "should", "now", "says",
        "report", "reports", "new", "amid", "after", "over", "into", "with", "us", "uk",
        "breaking", "live", "watch", "update", "exclusive", "first", "video", "photos"
    ])
    ...
    for c in candidates:
        ...
        for cluster in clusters:
            rep_subject = cluster[0].get("subject") or cluster[0].get("title") or ""
            rep_tokens = get_tokens(rep_subject)
            
            if not tokens or not rep_tokens:
                continue
                
            intersection = tokens.intersection(rep_tokens)
            union = tokens.union(rep_tokens)
            jaccard = len(intersection) / len(union) if union else 0.0
            
            if jaccard >= similarity_threshold or len(intersection) >= 2:
                cluster.append(c)
                placed = True
                break
    ...
    for cluster in clusters:
        if len(cluster) == 1:
            ...
            c["cluster_sources"] = [{"publisher": c.get("publisher", ""), "url": c.get("url", ""), "snippet": c.get("text", "")[:300]}]
            ...
            for c in cluster:
                pub = c.get("publisher") or urllib.parse.urlparse(c.get("url", "")).hostname or ""
                if pub.startswith("www."):
                    pub = pub[4:]
                if pub and pub not in seen_publishers:
                    seen_publishers.add(pub)
                    distinct_sources.append(c)
                elif not pub:
                    distinct_sources.append(c)
            ...
            if len(distinct_sources) == 1:
                c = distinct_sources[0]
                c["is_multi_source"] = False
                c["cluster_sources"] = [{"publisher": c.get("publisher", ""), "url": c.get("url", ""), "snippet": c.get("text", "")[:300]}]
                final_results.append(c)
                continue
                
            primary_c = distinct_sources[0].copy()
            primary_c["is_multi_source"] = True
            
            sources_summary = []
            dossier_lines = [f"=== MULTI-SOURCE TOPIC CLUSTER: {primary_c.get('subject')} ==="]
            
            for idx, s in enumerate(distinct_sources, 1):
                pub_name = s.get("publisher") or urllib.parse.urlparse(s.get("url", "")).hostname or f"Source {idx}"
                url = s.get("url", "")
                text = s.get("text", "")
                ...
                dossier_lines.append(f"\n[Source {idx} — {pub_name}] (URL: {url}):\n{text[:1200].strip()}")
                
            primary_c["cluster_sources"] = sources_summary
            primary_c["cross_source_dossier"] = "\n".join(dossier_lines)
            primary_c["text"] = f"Stated Claim / Post Context:\n{primary_c.get('subject')}\n\n{primary_c.get('cross_source_dossier')}"
            
            print(f"  [Cluster Created] '{primary_c.get('subject')}' ({len(distinct_sources)} distinct sources: {', '.join(seen_publishers)})")
            final_results.append(primary_c)
            multi_count += 1
            
    if multi_count > 0:
        print(f"\nClustered {len(candidates)} raw candidates into {len(final_results)} distinct topics ({multi_count} multi-source cluster(s)).")
...
COMMON_OUTLETS = {
    ...
    "11": ["dailymail.co.uk"],
    "12": ["nypost.com"],
    "13": ["washingtonexaminer.com"],
    "14": ["thefederalist.com"],
    "15": ["breitbart.com"]
}
...
def run_harvest():
    global TARGET_RSS, TARGET_BSKY, PREFERRED_OUTLET_DOMAINS
    parser = argparse.ArgumentParser(description='Harvest news candidates')
    parser.add_argument('--rss', '--rss-target', dest='rss_target', type=int, default=0)
    parser.add_argument('--bsky', '--bsky-target', dest='bsky_target', type=int, default=40)
    parser.add_argument('--prefer', type=str, default='')
    parser.add_argument('--category', type=str, default='all')
    parser.add_argument('--topic', type=str, default=None)
    parser.add_argument('--banned-topic', type=str, default=None)
    parser.add_argument('--enabled-feeds', type=str, default=None)
    args = parser.parse_args()
    TARGET_RSS = args.rss_target
    TARGET_BSKY = args.bsky_target
    if args.prefer:
        if args.prefer.strip().lower() in ('default', 'all'):
            for t, d in COMMON_OUTLETS.items(): PREFERRED_OUTLET_DOMAINS.extend(d)
        else:
            for t in args.prefer.split(','):
                t = t.strip().lower()
                if t in COMMON_OUTLETS: PREFERRED_OUTLET_DOMAINS.extend(COMMON_OUTLETS[t])
                elif t: PREFERRED_OUTLET_DOMAINS.append(t)
    ...
    # Only blocks the exact same article URL from being evaluated or harvested twice.
    # Multiple different outlets covering the same topic are preserved so they can be
    # grouped together into multi-source topic clusters by cluster_candidate_stories().
    seen_current_batch_urls = set()
    ...
    def is_duplicate_story(text, url, check_batch=True):
        ...
        if url_lower in seen_current_batch_urls:
            return True

        seen_current_batch_urls.add(url_lower)
        return False
    ...
    rss_feeds = [
        ...
        {"name": "Daily Mail Australia", "url": "https://www.dailymail.co.uk/auhome/index.rss", "categories": ["general"]},
        ...
        {"name": "Fox News Latest", "url": "http://feeds.foxnews.com/foxnews/latest", "categories": ["general", "world"]},
        {"name": "New York Post", "url": "https://nypost.com/news/feed/", "categories": ["general", "politics", "world"]},
        ...
        {"name": "Fox News Politics", "url": "http://feeds.foxnews.com/foxnews/politics", "categories": ["politics", "world", "general"]},
        {"name": "Washington Examiner", "url": "https://www.washingtonexaminer.com/feed/", "categories": ["politics", "general"]},
        {"name": "The Federalist", "url": "https://thefederalist.com/feed/", "categories": ["politics", "general"]},
        {"name": "Washington Times", "url": "https://www.washingtontimes.com/rss/headlines/news/politics/", "categories": ["politics", "general"]},
        {"name": "Daily Caller", "url": "https://dailycaller.com/feed/", "categories": ["politics", "general"]},
        {"name": "Breitbart", "url": "https://feeds.feedburner.com/breitbart", "categories": ["politics", "general"]},
    ...
    final_candidates = cluster_candidate_stories(existing_queue + successful_final)

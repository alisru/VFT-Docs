...
Execute the batch evaluation command. The agent uses the subagent spawning templates to delegate Index ranges (0-4, 5-9, etc.) to stateless evaluator sub-agents:
```bash
.venv\Scripts\python.exe bluesky_bot/orchestrate_batch_local.py
```
Each evaluator sub-agent:
1. Performs the 5-Phase Convergence Test on the story.
2. Compiles a 14-step JSON factcheck file (status set to `"COMPLETED DRY RUN"`) saved inside `bluesky_bot/stories/`.
3. Calls `draw_graph` from `generate_graph.py` to output trajectory graph images inside `bluesky_bot/graph_png/` and sync them to `_Generated_Content/graph_png/`.

...
### [2026-07-14] Intent 76: Batch Size Selection Controls and GUI Integration
*Status: Completed*
- **Modified CLI Parameter Default**: Changed the default `--chunk-size` (number of stories to process per API call) from `3` to `1` in `google_ai_studio_one_shot.py` to allow batching 1 story at a time by default.
- **Added GUI Controls**: Integrated a set of `ttk.Radiobutton` options ("1 at a time", "2 at a time", "3 at a time") under a new "Batch Size" row in `AletheiaLauncher.pyw`'s "One-Shot Batch Evaluator" card, defaulting to `1`.
- **Propagated Selection to CLI**: Configured the launcher's subprocess invoker to pass the selected batch size to the one-shot script via the `--chunk-size` parameter.
- **Strengthened Additional Context Injection**: Modified how the `--context` argument is fed to the model in `google_ai_studio_one_shot.py`. Rather than just placing it in the system instruction (which models often override/ignore during long formatting chains), it is now explicitly labeled with active instruction verbs in the system instruction AND injected directly into the user payload block (`=== ADDITIONAL CONTEXT / BACKGROUND TO CONSIDER ===`) right alongside the candidate news stories.
- **Fixed SON Registry Rebuilding & Coordinate Granularity**: Discovered that when running in SON mode, the batch evaluator script was calling the standard `rebuild_registries.py` script instead of `rebuild_registries_son.py`. Because the standard script lacks the formulaic force-attractor calculations (such as Separated Will and 18-variable normalizations), it was unable to recalculate and overwrite the coarse placeholder coordinates (`1.0`, `-1.0`) generated by the model. Introduced dynamic routing so that both the batch script and the operator console GUI invoke the correct rebuilder depending on the active SON flag, restoring granular floating-point coordinate resolution.
- **Morality-Will Audit**: (υ=+1.0, ψ=+1.5) -> Greater Good -> Operator control optimization, prompt attention layout improvements, and mathematical resolution recovery.

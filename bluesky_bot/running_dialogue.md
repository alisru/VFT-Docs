...
### [2026-08-15] Intent 77: Research Probe, Policy Auto-Tracking Ledger, Live Multimodal Chatbot with Persistent Memory & FastMCP Server
*Status: Completed*
- **Research Probe Mode (`research_probe.py`)**: Built a targeted historical and thematic harvester that searches Wikipedia REST summaries and DuckDuckGo HTML scraping for deep-context sources (e.g., `"Hitler 1933"`, `"Thatcher privatisation"`, `"Albanese housing policy 2024"`). Added `--probe` and `--probe-year` CLI parameters to `google_ai_studio_one_shot.py`, allowing operators to evaluate and judge historical events on the standard Psochic Hegemony coordinate grid.
- **Deterministic Policy Tracking (`policy_extract.py`)**: Created an automated regex extractor modeled after `actor_extract.py` that scans evaluated stories across 20+ core policy domains (Cost of Living, NDIS, Climate, Housing, Tariffs, Industrial Relations, AUKUS, Healthcare, etc.). Automatically updates running coordinate averages in `policy_ledger.json` on every registry rebuild.
- **Live LLM Chat Server (`chat_server.py`)**: Upgraded the local chat backend on port 8766 to support live API calls across Gemini (3.7 Flash, 3.5 Flash, 2.5 Flash, 2.5 Pro) and Google Cloud Vertex AI endpoints (`vertex:gemini-3.7-flash`, `vertex:gemini-3.5-flash`, `vertex:gemini-2.5-flash`, `vertex:gemini-3.1-flash-lite`). Includes in-memory RAG grounding over all 8,640+ stories and Google Search Grounding for live fact-checking.
- **Persistent Multi-Session Memory (`chat_sessions/`)**: Created a persistent JSON session management system with auto-saving across turns, long-term memory profiling (`memory_profile.json`), and instant switching between historical conversations in the sidebar.
- **Multimodal Document & File Upload Tools (`chat_uploads/`)**: Integrated drag-and-drop and file attachment support for PDF, text, markdown, JSON, and image files. The backend extracts document contents or passes images directly to the Gemini/Vertex vision pipeline for live Actualism auditing and coordinate mapping.
- **Research Probe Mode Bugfix (`google_ai_studio_one_shot.py`)**: Fixed the scoped `UnboundLocalError` on `json.load(f)` when running `--probe` queries, enabling one-shot batch evaluations to harvest and evaluate historical and policy topics smoothly.
- **Control Panel Integration & Status Fix (`control_panel.html`)**: Resolved the offline badge bug by replacing the 40MB loop fetch with lightweight memory/health polling, and added a direct **"💬 Open AI Chatbot"** launch button to the top header bar.
- **Dynamic MCP Server Expansion (`aletheia_mcp_server.py`)**: The MCP server dynamically loads live story files on every call with zero restart needed. Expanded with full analytical tools: `get_corpus_overview()`, `get_gap_distribution()`, `get_actor_hypocrisy_leaderboard()`, `get_outlet_hypocrisy_leaderboard()`, `get_worst_divergence_stories()`, and `get_pass_stories()`.
- **Static Audit Site Exporter (`export_audit_site.py`)**: Built a zero-dependency static site builder that bundles `index.html`, all 8,651+ stories, charts, `hegemony_db.js`, 10,641+ graph images, and `chat.html` into `bluesky_bot/dist_audit_site/` ready for instant deployment to GitHub Pages, Cloudflare Pages, Netlify, or Vercel.
- **Morality-Will Audit**: (υ=+1.8, ψ=+1.9) -> Systemic Justice / Productive Justice. Transforming the bot into an open, auditable public intelligence platform.

### Intent 78: Hierarchical Topic Threading, Local SQLite Archive Memory & 2-MCP Modular Architecture
- **Local SQLite / FTS5 Memory Engine (`bluesky_bot/memory_store.py`)**: Built a zero-dependency local SQLite WAL database that automatically indexed 127 markdown & text discussion files from `_AI files and chat logs/` into 2,204 granular chunks for sub-millisecond BM25 keyword and theoretical quote retrieval.
- **Hierarchical Topic Threading (`bluesky_bot/chat_server.py`)**: Upgraded the chat backend to support conversation branching trees (`threads: { "main": ..., "thread_xxx": ... }`), ancestral inheritance from root up to fork point, and one-click synthesis merging (`POST /session/thread/merge`) to generate executive takeaway cards and update the session's narrative spine.
- **Interactive UI Thread Navigator (`bluesky_bot/aletheia_chat.html`)**: Built a breadcrumb header (`# Main Trunk` > `↳ 🏠 Housing Policy`), sidebar nested branch tree, message fork button (`🧵 Branch Thread`), and live grounding badges for audits and archive logs.
- **Modular 2-MCP Server Architecture**:
  - `aletheia_mcp_server.py`: Dedicated Fact-Checking & Conversation Memory server (8,651+ audits, hypocrisy leaderboards, policy ledgers, chat threads).
  - `Semantic_Clusters/vft_mcp_server.py`: Dedicated VFT Corpus & Vector DB server (Qdrant `vft_paragraphs` vector search, 127+ archive chat logs, `topic_ism_mapping.json`).
- **Morality-Will Audit**: (υ=+1.9, ψ=+1.9) -> Systemic Justice / Productive Justice. Giving the operator infinite branching research memory with zero context dilution and total conceptual grounding across the entire VFT repository.

### Intent 101: Story Schema Invariants & Pre-Flight Validation Fixes
- **Root Cause of Quarantine Failures**:
  - The pre-flight validator (`validate_batch.py`) strictly requires top-level keys `mode` and `status`, plus `multiAspect: True` whenever `posts` has length 14.
  - When draft stories were compiled via `/story/generate_from_chat` or `/story/save_draft`, `mode`, `status`, and `multiAspect` were not explicitly injected, causing `validate_batch.py` to quarantine them into `stories/fail/`.
- **System-Wide Remediation**:
  - Updated `chat_server.py` (`/story/generate_from_chat` and `/story/save_draft`) to unconditionally enforce and populate:
    - `"mode": "root"` (or user specified)
    - `"status": "COMPLETED DRY RUN"`
    - `"multiAspect": True` (when 14 posts)

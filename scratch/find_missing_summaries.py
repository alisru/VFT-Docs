...
vft_md_dir = os.path.join(vft_root, "_VFT MD")
...
# 2. Walk _VFT MD to find all .md files
missing_files = []
all_md_files = []
for root, dirs, files in os.walk(vft_md_dir):
    # Skip _Archive or hidden dirs if any
...
print(f"Total .md files in _VFT MD: {len(all_md_files)}")
print(f"Missing from file_summaries.md: {len(missing_files)}")

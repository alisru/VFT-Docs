...
- API key flow (Anthropic API direct calls) — requires separate account, billing, key management; clipboard+DOM-read is sufficient
- Per-project aggregate summary generation — per-chat is the correct granularity; project view is index only
- Type and date sort options — type is noise, dates not in DOM for generated artifacts
- Sorting or injecting into uploads or project file sections

...
def gdelt_bigquery_search(actor_name, date_from=None, date_to=None, max_records=500):
    ...
    try:
        ...
        print(f"  [error] Couldn't create BigQuery client (check GCP auth / project): {e}", file=sys.stderr)
        return None
...
def scrape_article(article_meta, actor_name):
...
def main():
    ...
    # Resume support: skip URLs we've already extracted quotes from.
    seen_urls = set()
    ...
    with open(out_path, file_mode, encoding="utf-8") as f:
        for i, art in enumerate(new_urls, 1):
            ...
            try:
                records = scrape_article(art, args.name)
            except Exception as e:

...
## Local Sourcing Tools for Australian Federal Material

If a founder/iconic-figure quote for the Australian Kanon is a federal politician's parliamentary statement, check these local tools before a live search:

- `query_hansard_corpus.py` -- queries the local `corpus_1998_to_2025.parquet` via duckdb. House of Representatives, full speech text, 1998-03-02 to 2025-07-31, instant, no network cost. First stop for anything said in the chamber in that range.
- `aph_scraper.py` -- live scrape of aph.gov.au's own Hansard search, no result cap, covers Senate + House + committees + full history back well before 1998. Use for anything outside the parquet's coverage.

Both are in `Aus Kanon/compact JSON/`. For historical figures/events well outside the federal-politician case (e.g. Federation-era documents, non-parliamentary national quotes), these tools don't apply -- use the normal training-data-hypothesis-then-verify sequence below.

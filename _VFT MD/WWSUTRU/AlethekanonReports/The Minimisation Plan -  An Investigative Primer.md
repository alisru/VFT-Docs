...
**Identifying the Fake Maximiser:** A 'Fake Maximiser' is an actor whose strategic output consistently maps to the 'Greater Lie' quadrant (−υ,+ψ)—proactive policies with extractive or insufficient outcomes—while being framed under the 'Cover' of the 'Greater Good' (+υ,+ψ). The defining characteristic is the persistent contradiction between their stated Maximiser goals and the observable Minimiser effects or compromises of their actions. This can be quantified by the distance between the 'Framed Vector' (the policy's public narrative) and the 'True Intent Vector' (the policy's actual systemic impact) on the Psochic Hegemony.

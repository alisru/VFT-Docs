...
- Add `running_dialogue.md` (this file).
- Update the public template (`bluesky_bot_instructions.md`) and scripts to mandate Nuance (bright side/poison), explicit Evidence Standards, explicit Stated vs Actual judgements, and the updated coordinate labels (`+1,0` = Good Preference, `-1,0` = Bad Preference).
- Perform a live post (using the app password) applying these new rules. (We will verify with the user if they want the LA Times story or a new RSS story).
